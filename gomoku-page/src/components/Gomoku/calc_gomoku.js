...
function searchDLine(cells) {
  ...
  const fill0List = r.map(() => 0);
  const fillSizeList = r.map(() => boardSize);
  ...
  for (var q of l2) {
    const l1 = createDLine1(q, cells, boardSize);
    if (hasLine(l1)) return true;
  }

  for (var s of l3) {
    const l2 = createDLine2(s, cells, boardSize);
...
function createDLine1(p, cells, size) {
  ...
  return _.range(size)
    .map(i => {
      return isSizeRange(n + i, size) && isSizeRange(m + i, size)
        ? [n + i, m + i]
        : undefined;
    })
    .filter(i => !_.isNil(i))
